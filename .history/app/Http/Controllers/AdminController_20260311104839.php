    ...
    public function moverManutencao(Request $request, $id)
    {
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 1. LÓGICA DE MOVIMENTAÇÃO DE CRÉDITO (CORRIGIDA 🚀)
                if ($valorOriginal > 0 && ($action === 'transfer' || $action === 'credit')) {
                    ...
                    foreach ($proximasReservas as $proxima) {
                        ...
                        if ($saldoDevedor > 0) {
                            ...
                            $novoTotalPago = round($jaPago + $valorInjetado, 2);

                            // Atualiza a reserva de destino
                            ...
                            // ✨ O PULO DO GATO: Além de mudar o reserva_id, mudamos o paid_at para a data do novo jogo
                            // Isso faz o dinheiro sair do caixa de hoje e entrar no caixa do dia futuro
                            DB::table('financial_transactions')
                                ->where('reserva_id', $reserva->id)
                                ->update([
                                    'reserva_id' => $proxima->id,
                                    'paid_at'    => $proxima->date . ' ' . date('H:i:s') // 🕒 Nova data contábil
                                ]);
                ...
                // 2. ESTORNO (Caso não queira transferir ou não tenha próximo jogo)
                if ($valorOriginal > 0 && ($action === 'refund' || !$transferenciaSucesso)) {
                    $dataOperacional = $reserva->date instanceof \DateTime ? $reserva->date->format('Y-m-d') : date('Y-m-d', strtotime($reserva->date));

                    FinancialTransaction::create([
                        'reserva_id'     => null,
                        'arena_id'       => $reserva->arena_id,
                        'user_id'        => $reserva->user_id,
                        'manager_id'     => auth()->id() ?? $reserva->manager_id,
                ...
                $msg = "Olá {$nomeLimpo}! 👋\n\nInformamos que o seu horário do dia {$dataReservaFormatada} às {$horaReserva} precisou ser interrompido para MANUTENÇÃO ({$motivo}).";
                if ($valorOriginal > 0) {
                    $valorF = number_format($valorOriginal, 2, ',', '.');
                    $msg .= $transferenciaSucesso ? "\n\n⭐ R$ {$valorF} movido para crédito no dia {$reservaDestinoData}." : "\n\n💰 R$ {$valorF} estornado.";
                }
                $waLink = "https://wa.me/55" . preg_replace('/\D/', '', $contatoOriginal) . "?text=" . urlencode($msg);

                return response()->json(['success' => true, 'message' => 'Manutenção registrada!', 'whatsapp_link' => $waLink]);
            } catch (\Exception $e) {
