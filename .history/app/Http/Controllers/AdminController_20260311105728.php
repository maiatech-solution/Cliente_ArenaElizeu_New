                ...
                if ($valorOriginal > 0 && ($action === 'transfer' || $action === 'credit')) {
                    ...
                    foreach ($proximasReservas as $proxima) {
                        ...
                        if ($saldoDevedor > 0) {
                            ...
                            // Converte a data para string para evitar erro 500
                            $novaDataContabil = date('Y-m-d', strtotime($proxima->date));
                            ...
                            // ✨ SOLUÇÃO: Atualiza a transação com data formatada
                            DB::table('financial_transactions')
                                ->where('reserva_id', $reserva->id)
                                ->update([
                                    'reserva_id' => $proxima->id,
                                    'paid_at'    => $novaDataContabil . ' ' . date('H:i:s')
                                ]);
