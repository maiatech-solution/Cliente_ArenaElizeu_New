    ...
    public function moverManutencao(Request $request, $id)
    {
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                $valorOriginal = round((float) $reserva->total_paid, 2);
                ...
                // 1. LÓGICA DE MOVIMENTAÇÃO DE CRÉDITO (VERSÃO ESTÁVEL 🚀)
                if ($valorOriginal > 0 && ($action === 'transfer' || $action === 'credit')) {
                    ...
                    foreach ($proximasReservas as $proxima) {
                        // ✨ PASSO A: Move a transação financeira primeiro (Única fonte da verdade)
                        ...
                        // ✨ PASSO B: Recalcula o total pago REAL baseado no banco para evitar duplicação (90 reais)
                        $realPaid = DB::table('financial_transactions')
                            ->where('reserva_id', $proxima->id)
                            ->sum('amount');

                        // ✨ PASSO C: Atualiza a reserva de destino com o valor cravado
                        DB::table('reservas')->where('id', $proxima->id)->update([
                            'total_paid'     => $realPaid,
                            'signal_value'   => $realPaid,
                            'payment_status' => ($realPaid >= (float)$proxima->price) ? 'paid' : 'partial',
                        ...
                        break; // Move o crédito para a próxima imediata
                    }
                }

                // 2. ESTORNO (Caso solicitado ou falha na transferência)
                if ($valorOriginal > 0 && ($action === 'refund' || !$transferenciaSucesso)) {
                    ...
                    FinancialTransaction::create([
                        ...
                        'description'    => "ESTORNO (Manutenção #{$reserva->id}): " . $motivo,
                        'paid_at'        => $dataOperacional . ' ' . now()->format('H:i:s'),
                    ]);
                }

                // 3. FINALIZAÇÃO: Zera a reserva atual
                $reservaDestinoData = !empty($datasDestino) ? implode(', ', array_unique($datasDestino)) : null;
                $backupData = [
                    'name' => $nomeLimpo,
                    'status_orig' => $reserva->status,
                    'valor_transferido' => $transferenciaSucesso ? $valorOriginal : 0
                ];
                ...
                DB::table('reservas')->where('id', $id)->update([
                    ...
                    'signal_value' => 0,
                    'is_fixed' => 1,
                    'notes' => "###BACKUP###" . json_encode($backupData) . "###END###\n" . ($reserva->notes ?? '')
                ]);

                // Mensagem de WhatsApp
                $msg = "Olá {$nomeLimpo}! 👋\n\nSeu horário de hoje ({$dataReservaFormatada} às {$horaReserva}) foi movido para MANUTENÇÃO ({$motivo}).";
                if ($valorOriginal > 0) {
                    $valorF = number_format($valorOriginal, 2, ',', '.');
                    $msg .= $transferenciaSucesso ? "\n\n⭐ R$ {$valorF} foi movido para crédito no dia {$reservaDestinoData}." : "\n\n💰 R$ {$valorF} foi estornado.";
                }
                ...
                \Log::error("❌ ERRO MANUTENÇÃO: " . $e->getMessage());
                return response()->json(['success' => false, 'message' => 'Erro interno ao processar financeiro.'], 500);
            }
        });
    }
