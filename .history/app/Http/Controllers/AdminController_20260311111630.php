...
class AdminController extends Controller
{
    ...
    {
        // 🛡️ TRAVA DE SEGURANÇA: Validação de Supervisor
        if (auth()->user()->role === 'colaborador') {
            $supervisorEmail = $request->input('supervisor_token');
            $supervisor = \App\Models\User::where('email', $supervisorEmail)
                ->whereIn('role', ['admin', 'gestor'])
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 💰 Fonte da verdade: Pegamos o valor real baseado apenas em transações de ENTRADA (positivas)
                $valorRealTransacoes = DB::table('financial_transactions')
                    ->where('reserva_id', $reserva->id)
                    ->where('amount', '>', 0)
                    ->sum('amount');

                $nomeLimpo = str_replace(['🛠️ MANUTENÇÃO (', ')'], '', $reserva->client_name);
                $contatoOriginal = $reserva->client_contact;
                $userIdOriginal = $reserva->user_id;
                $dataReservaFormatada = date('d/m', strtotime($reserva->date));
                $horaReserva = date('H:i', strtotime($reserva->start_time));

                $transferenciaSucesso = false;
                $datasDestino = [];

                // 1. LÓGICA DE MOVIMENTAÇÃO DE CRÉDITO (VERSÃO DEFINITIVA 🛡️)
                if ($valorRealTransacoes > 0 && ($action === 'transfer' || $action === 'credit')) {
                    ...
                    if ($proxima) {
                        // ✨ PASSO A: Move TODAS as transações positivas (dinheiro vivo) para a próxima reserva
                        DB::table('financial_transactions')
                            ->where('reserva_id', $reserva->id)
                            ->where('amount', '>', 0)
                            ->update([
                                'reserva_id' => $proxima->id,
                                'paid_at'    => date('Y-m-d', strtotime($proxima->date)) . ' ' . date('H:i:s')
                            ]);

                        // ✨ PASSO B: Recalcula o total pago REAL na reserva de destino para evitar duplicações
                        $totalAtualizadoDestino = DB::table('financial_transactions')
                            ->where('reserva_id', $proxima->id)
                            ->sum('amount');

                        DB::table('reservas')->where('id', $proxima->id)->update([
                            'total_paid'     => $totalAtualizadoDestino,
                            'signal_value'   => $totalAtualizadoDestino,
                            'payment_status' => ($totalAtualizadoDestino >= (float)$proxima->price) ? 'paid' : 'partial',
                ...
                // 2. ESTORNO (Se solicitado ou se não houver reserva para transferir)
                if ($valorRealTransacoes > 0 && ($action === 'refund' || !$transferenciaSucesso)) {
                    $dataOperacional = date('Y-m-d', strtotime($reserva->date));
                ...
                // 3. FINALIZAÇÃO: Zera a reserva atual e marca como Manutenção
                $backupData = [
                    'name' => $nomeLimpo,
                    'valor_original' => $valorRealTransacoes,
                    'finance_action' => $transferenciaSucesso ? 'transferred' : 'refunded'
                ];
                ...
                DB::table('reservas')->where('id', $id)->update([
                    ...
                    'signal_value' => 0,
                    'is_fixed' => 1,
                    'notes' => "###FIN_BACKUP###" . json_encode($backupData) . "###END###\n" . ($reserva->notes ?? '')
                ]);

                // Resposta para WhatsApp
                $destLabel = !empty($datasDestino) ? implode(', ', $datasDestino) : '';
                $msg = "Olá {$nomeLimpo}! 👋\n\nInformamos que seu horário do dia {$dataReservaFormatada} às {$horaReserva} foi movido para MANUTENÇÃO ({$motivo}).";
                if ($valorRealTransacoes > 0) {
                    $valorF = number_format($valorRealTransacoes, 2, ',', '.');
                    $msg .= $transferenciaSucesso ? "\n\n⭐ R$ {$valorF} foi movido para crédito no dia {$destLabel}." : "\n\n💰 R$ {$valorF} foi estornado.";
                }
                $waLink = "https://wa.me/55" . preg_replace('/\D/', '', $contatoOriginal) . "?text=" . urlencode($msg);

                return response()->json(['success' => true, 'message' => 'Manutenção aplicada com sucesso!', 'whatsapp_link' => $waLink]);
            } catch (\Exception $e) {
                \Log::error("❌ ERRO CRÍTICO MOVER MANUTENÇÃO: " . $e->getMessage());
                return response()->json(['success' => false, 'message' => 'Erro técnico ao processar financeiro.'], 500);
            }
        });
    }
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                if ($decisao === 'release_slot' || empty($decisao)) {
                    ...
                    return redirect()->back()->with('success', '✅ Agenda liberada com sucesso!');
                }

                // --- CASO 2: RESTAURAR O CLIENTE ORIGINAL (BACKUP INTELIGENTE) ---
                if ($decisao === 'restore_client') {
                    $notes = $reserva->notes;

                    // Regex flexível para capturar ###BACKUP### ou ###FIN_BACKUP###
                    if (preg_match('/###(?:FIN_)?BACKUP###(.*?)###END###/s', $notes, $matches)) {
                        $dados = json_decode($matches[1], true);

                        if (!$dados) {
                            return redirect()->back()->with('error', '⚠️ Erro: Dados de backup corrompidos.');
                        }

                        // Mapeamento dinâmico de chaves (ajusta versões diferentes do backup)
                        $nomeCliente    = $dados['name'] ?? $dados['orig_client'] ?? 'Cliente';
                        $valorOriginal  = (float) ($dados['total_paid_orig'] ?? $dados['valor_movido'] ?? $dados['valor_original'] ?? 0);
                        ...
                        // 1. Restaura os dados básicos e remove o backup das notas
                        $reserva->update([
                        ...
                        // 2. Sincroniza o Financeiro (A única verdade)
                        // Se o dinheiro foi devolvido ou movido, o total_paid aqui DEVE ser o que está no banco agora
                        $realPaid = DB::table('financial_transactions')
                            ->where('reserva_id', $reserva->id)
                            ->where('amount', '>', 0)
                            ->sum('amount');
                        ...
                        $reserva->update([
                            'total_paid'     => $realPaid,
                            'signal_value'   => $realPaid,
                            'payment_status' => ($realPaid >= (float)$reserva->price) ? 'paid' : ($realPaid > 0 ? 'partial' : 'unpaid')
                        ]);

                        // --- 🚀 MENSAGEM WHATSAPP ---
                        $dataReserva = date('d/m', strtotime($reserva->date));
                        $horaReserva = date('H:i', strtotime($reserva->start_time));
                        $msg = "Boas notícias {$nomeCliente}! 👋\n\nA manutenção foi concluída e seu horário de hoje ({$dataReserva} às {$horaReserva}) foi REATIVADO! 🏟️";

                        if ($valorOriginal > 0) {
                            $valorF = number_format($valorOriginal, 2, ',', '.');
                            if ($acaoRealizada === 'credit' || $acaoRealizada === 'transferred') {
                                $msg .= "\n\n⭐ *PAGAMENTO:* O valor de R$ {$valorF} foi movido como crédito para seu próximo jogo. Para hoje, o acerto será integral.";
                            } else {
                                $msg .= "\n\n💰 *PAGAMENTO:* Como realizamos o estorno do sinal, o pagamento integral fica para o momento do jogo.";
                            }
                        }

                        $waLink = "https://wa.me/55" . preg_replace('/\D/', '', $contato) . "?text=" . urlencode($msg);

                        return redirect()->route('admin.reservas.show', $reserva->id)->with([
                            'success'       => '👤 Cliente restaurado e saldo sincronizado!',
                            'whatsapp_link' => $waLink
                        ]);
                    }

                    return redirect()->back()->with('error', '⚠️ Falha: Backup não encontrado nas notas.');
                }
