        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                if ($valorRealTransacoes > 0 && ($action === 'transfer' || $action === 'credit')) {
                    ...
                    if ($proxima) {
                        // ✨ PASSO A: Move TODAS as transações positivas para a próxima reserva
                        ...
                        // ✨ PASSO B: Recalcula o total pago REAL na reserva de destino
                ...
                if ($valorRealTransacoes > 0 && ($action === 'refund' || !$transferenciaSucesso)) {
                    ...
                    FinancialTransaction::create([
                        'reserva_id'     => $reserva->id, // 🎯 CORREÇÃO: Vincula o estorno à reserva para o balanço zerar
                ...
                $backupData = [
                    ...
                    'user_id' => $userIdOriginal,
                    'total_paid_orig' => $valorRealTransacoes,
                    'finance_action' => $transferenciaSucesso ? 'credit' : 'refund',
                    'dest_date' => !empty($datasDestino) ? implode(', ', $datasDestino) : null
                ];
            ...
            try {
                ...
                if ($decisao === 'restore_client') {
                    ...
                    if (preg_match('/###(?:FIN_)?BACKUP###(.*?)###END###/s', $notes, $matches)) {
                        ...
                        // 1. Restaura os dados básicos e remove o bloco de backup das notas
                        $reserva->update([
                        ...
                        // 🔴 CORREÇÃO: Removemos o filtro de 'amount > 0' para que Entradas + Estornos resultem no saldo real
                        ...
                        if ($valorOriginal > 0) {
                            ...
                            if ($acaoRealizada === 'credit' || $acaoRealizada === 'transferred') {
                                ...
                                $msg .= "\n\n💰 *PAGAMENTO:* Como realizamos o estorno do sinal, o pagamento integral fica pendente para o momento do jogo.";
                            }
                        }
