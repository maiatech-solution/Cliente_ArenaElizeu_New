    ...
    public function moverManutencao(Request $request, $id)
    {
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 💰 Fonte da verdade (Apenas entradas REAIS)
                ...
                if ($valorRealTransacoes > 0 && ($action === 'transfer' || $action === 'credit')) {
                    ...
                    })->where('id', '!=', $reserva->id)->where('date', '>', $reserva->date)
                    ...
                    if ($proxima) {
                        DB::table('financial_transactions')
                            ...
                            ->update([
                                ...
                                'paid_at'    => $proxima->date . ' ' . date('H:i:s')
                            ]);

                        $totalDestino = DB::table('financial_transactions')->where('reserva_id', $proxima->id)->sum('amount');
                        $proxima->update([
                            'total_paid' => $totalDestino,
                            'payment_status' => ($totalDestino >= $proxima->price) ? 'paid' : 'partial',
                            'status' => 'confirmed'
                        ]);
                ...
                // 2. ESTORNO (Com trava de duplicidade 🛡️)
                if ($valorRealTransacoes > 0 && ($action === 'refund' || !$transferenciaSucesso)) {
                    $jaEstornado = FinancialTransaction::where('reserva_id', $reserva->id)
                        ->where('type', 'refund')->exists();

                    if (!$jaEstornado) {
                        FinancialTransaction::create([
                            'reserva_id'     => $reserva->id, // Vínculo para saldo zerar
                            ...
                            'description'    => "ESTORNO ÚNICO (Manutenção #{$reserva->id})",
                            'paid_at'        => now(),
                        ]);
                    }
                }
                ...
                // 3. FINALIZAÇÃO
                $backupData = [
                    'name' => str_replace(['🛠️ MANUTENÇÃO (', ')'], '', $reserva->client_name),
                    'contact' => $reserva->client_contact,
                    'user_id' => $reserva->user_id,
                ...
                $reserva->update([
                    'status' => 'maintenance',
                    'client_name' => "🛠️ MANUTENÇÃO (" . $backupData['name'] . ")",
                    'total_paid' => 0,
                    'signal_value' => 0,
                    'is_fixed' => 1,
                ...
                return response()->json(['success' => false, 'message' => 'Erro técnico.'], 500);
            }
        });
    }
