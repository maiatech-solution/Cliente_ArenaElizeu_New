    ...
    public function moverManutencao(Request $request, $id)
    {
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 💰 SALDO ATUAL: Apenas o que entrou (+) nesta reserva específica
                $saldoLiquido = (float) DB::table('financial_transactions')
                ...
                if ($saldoLiquido > 0 && ($action === 'transfer' || $action === 'credit')) {
                    ...
                    if ($proxima) {
                        // ✨ PASSO ZERO: Limpeza de Terreno
                        // Se a reserva de destino já tiver transações (lixo de testes), desvinculamos elas
                        ...
                        // ✨ PASSO A: Migração Atômica
                        DB::table('financial_transactions')
                            ->where('reserva_id', $reserva->id)
                            ->where('amount', '>', 0)
                            ->update([
                                'reserva_id' => $proxima->id,
                                'paid_at'    => $apenasDataDestino . ' ' . date('H:i:s')
                            ]);

                        // ✨ PASSO B: Recálculo Baseado em Fatos (Extrato)
                ...
                // 6. Estorno com Vínculo (Para o visor zerar)
                if ($saldoLiquido > 0 && ($action === 'refund' || !$transferenciaSucesso)) {
                ...
                return response()->json(['success' => true, 'message' => 'Operação concluída.']);
            } catch (\Exception $e) {
                \Log::error("❌ ERRO CRÍTICO MOVER MANUTENCAO: " . $e->getMessage());
