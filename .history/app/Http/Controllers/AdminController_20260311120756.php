    ...
    public function moverManutencao(Request $request, $id)
    {
        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                // 💰 SALDO REAL HOJE: Soma tudo que está vinculado a esta reserva (Entradas e Estornos)
                ...
                if ($saldoAtualExtrato > 0 && ($action === 'transfer' || $action === 'credit')) {
                    ...
                    if ($proxima) {
                        // 🔥 PASSO ZERO: FAXINA NO DESTINO (O segredo para não dar 80 reais)
                        // Removemos qualquer transação que já esteja na reserva do dia 18 (Sinais automáticos, lixo, etc)
                        ...
                        // 🚀 PASSO A: MIGRAÇÃO DO CRÉDITO
                        // Movemos apenas as transações POSITIVAS da reserva atual para a próxima
                        DB::table('financial_transactions')
                            ->where('reserva_id', $reserva->id)
                            ->where('amount', '>', 0)
                            ->update([
                        ...
                        // 📊 PASSO B: SINCRONIZAÇÃO POR SOMA (Sem achismo)
                ...
                // 3. LÓGICA DE ESTORNO (Se solicitado ou se não houver próximo para transferir)
                ...
                // 4. FINALIZAÇÃO DA RESERVA ATUAL (Sempre fica zerada)
                $backup = [
                    'name' => $nomeLimpo,
                    'contact' => $contato,
                    'user_id' => $userId,
            ...
            try {
                ...
                if ($decisao === 'restore_client') {
                    ...
                    if (preg_match('/###(?:FIN_)?BACKUP###(.*?)###END###/s', $notes, $matches)) {
                        ...
                        // Somamos TUDO: Entradas (+) e Estornos (-)
                        $realPaid = (float) DB::table('financial_transactions')
                            ->where('reserva_id', $reserva->id)
                            ->sum('amount');

                        // 🛡️ TRAVA DE EXIBIÇÃO: O visor da reserva não pode mostrar saldo negativo.
                        // Se o saldo real for -40 (estornou e não tinha saldo), mostramos 0 no visor.
