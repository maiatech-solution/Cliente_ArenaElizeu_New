        ...
        return DB::transaction(function () use ($request, $id) {
            try {
                ...
                if ($saldoAtualExtrato > 0 && ($action === 'transfer' || $action === 'credit')) {
                    ...
                    if ($proxima) {
                        // 🔥 PASSO ZERO: FAXINA NO DESTINO (O "Anti-Duplicidade")
                        // Deletamos o sinal automático que o sistema criou para a reserva futura
                        // para que ele não se some ao crédito real que estamos movendo agora.
                        DB::table('financial_transactions')
                            ->where('reserva_id', $proxima->id)
                            ->where('type', 'signal') // Remove o "placeholder" de 50 reais
                            ->delete();

                        // Desvinculamos qualquer outro lixo que tenha sobrado no ID da próxima
                        DB::table('financial_transactions')
                            ->where('reserva_id', $proxima->id)
                        ...
                        // Movemos as transações POSITIVAS da reserva atual para a próxima
                        DB::table('financial_transactions')
                            ->where('reserva_id', $reserva->id)
                            ->where('amount', '>', 0)
                            ->update([
                        ...
                        // 📊 PASSO B: SINCRONIZAÇÃO POR SOMA REAL (Sem achismo)
