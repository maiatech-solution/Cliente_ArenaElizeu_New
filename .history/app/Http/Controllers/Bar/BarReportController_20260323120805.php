...
class BarReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // 🎯 A REGRA DE OURO: Tudo o que é financeiro vem da BarCashMovement
        $queryMovimentos = BarCashMovement::whereBetween('created_at', [$startDate, $endDate]);

        // Se não for admin, ele só vê a performance dele
        if (!$isAdmin) {
        ...
        // 1. Faturamento Consolidado (Vendas - Estornos)
        ...
        // 3. Contagem de Itens Vendidos (Vinculado aos movimentos de caixa existentes)
        // Buscamos os itens apenas das ordens/vendas que de fato geraram movimentação financeira
        $itensMesas = BarOrderItem::whereHas('order.movements', function ($q) use ($startDate, $endDate, $isAdmin, $user) {
            $q->whereBetween('created_at', [$startDate, $endDate]);
            if (!$isAdmin) $q->where('user_id', $user->id);
        })->sum('quantity');

        $itensPDV = BarSaleItem::whereHas('sale.movements', function ($q) use ($startDate, $endDate, $isAdmin, $user) {
            $q->whereBetween('created_at', [$startDate, $endDate]);
            if (!$isAdmin) $q->where('user_id', $user->id);
        })->sum('quantity');
    ...
    public function daily(Request $request)
    {
        ...
        // 1. Busca Pedidos de Mesas
        $orderItems = \App\Models\Bar\BarOrderItem::whereHas('order', function ($q) use ($startDate, $endDate) {
            $q->whereIn('status', ['paid', 'pago'])
                ->whereBetween('updated_at', [$startDate, $endDate]);
        })->with('product')->get();

        // 2. Busca Vendas de PDV
        $saleItems = \App\Models\Bar\BarSaleItem::whereHas('sale', function ($q) use ($startDate, $endDate) {
            $q->whereIn('status', ['paid', 'pago'])
        ...
        // 5. Processa Itens de PDV
        foreach ($saleItems as $item) {
            $dia = $item->created_at->format('Y-m-d');
            if (isset($datas[$dia])) {
                // No PDV usamos price_at_sale se houver, ou unit_price
