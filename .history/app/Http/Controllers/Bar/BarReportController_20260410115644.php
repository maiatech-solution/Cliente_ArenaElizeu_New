...
class BarReportController extends Controller
{
    ...
    public function vouchers(Request $request)
    {
        ...
        $cortesias = $query->orderBy('created_at', 'desc')->get()->map(function ($item) {
            // 🎯 O SEGREDO: Se o movimento financeiro for 0, pegamos o valor real do pedido
            if ($item->amount <= 0 && $item->barOrder) {
                $item->valor_real = $item->barOrder->total_value;
            } else {
                $item->valor_real = $item->amount;
            }
            return $item;
        });
