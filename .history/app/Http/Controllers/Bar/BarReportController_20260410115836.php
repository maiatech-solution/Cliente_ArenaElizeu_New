...
class BarReportController extends Controller
{
    ...
    public function vouchers(Request $request)
    {
        ...
        // 🎯 O NOME CORRETO DA MODEL É BarCashMovement (conforme sua CashController)
        // Filtramos apenas movimentações do tipo 'venda' feitas com 'voucher'
        $query = \App\Models\Bar\BarCashMovement::with(['user', 'barOrder'])
            ->where('type', 'venda')
            ->where('payment_method', 'voucher')
            ->whereBetween('created_at', [$start . ' 00:00:00', $end . ' 23:59:59'])
            ->when($search, function ($q) use ($search) {
                $q->whereHas('barOrder', function ($sub) use ($search) {
        ...
        // Na sua BarCashMovement o campo de valor é 'amount'
        $totalValor = $cortesias->sum('amount');
