...
class BarReportController extends Controller
{
    ...
    public function vouchers(Request $request)
    {
        ...
        $cortesias = $query->orderBy('created_at', 'desc')->get()->map(function ($item) {
            ...
            // Como não tem 'bar_sale_id', buscamos uma venda de PDV no mesmo minuto/usuário
            if ($valorReal <= 0) {
                $valorReal = \DB::table('bar_sales')
                    ->where('user_id', $item->user_id)
                    ->where('payment_method', 'voucher')
                    // Margem de 2 segundos para garantir o sincronismo
            ...
            // 3. Se ainda for zero, usa o que estiver no amount (fallback)
