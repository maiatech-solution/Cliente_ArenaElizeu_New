...
class FinanceiroController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // 📅 2. Ocupação Filtrada (Total de Jogos)
        $totalReservasMes = Reserva::whereMonth('date', $mes)
            ...
            ->whereIn('status', [
                ...
                'completed' // Garantindo que status 'completed' também conte como jogo realizado
            ])
            ->when($arenaId, fn($q) => $q->where('arena_id', $arenaId))
            ->count();

        // 🚫 3. Faltas Filtradas (No-Show) - PRECISÃO TOTAL
        // Conta reservas marcadas como No-Show, mesmo que tenham sido canceladas/estornadas depois
        $reservasNoShowCount = Reserva::whereMonth('date', $mes)
            ->whereYear('date', $ano)
            ->whereIn('status', [
                Reserva::STATUS_NO_SHOW,
                'canceled' // Inclui canceladas, pois faltas estornadas costumam mudar para canceled
            ])
            ->when($arenaId, fn($q) => $q->where('arena_id', $arenaId))
            ->where(function ($q) {
                // Filtro adicional opcional: garante que se for canceled, tenha vindo de um no-show
                // Se preferir contar todos os cancelamentos como falta, pode remover este sub-where
            })
            ->count();

        // Conta registros financeiros de multa (inclusive estornos e valores retidos R$ 0,00)
        $transacoesMultaCount = FinancialTransaction::whereMonth('paid_at', $mes)
            ...
            ->where(function ($q) {
                $q->where('description', 'like', '%Multa de Falta%')
                    ...
                    ->orWhere('description', 'like', '%Falta marcada%');
            })
            ->when($arenaId, fn($q) => $q->where('arena_id', $arenaId))
            ->count();

        // Soma as reservas de falta + multas manuais do caixa
