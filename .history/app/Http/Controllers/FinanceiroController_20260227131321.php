...
class FinanceiroController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Força a referência para o mês atual caso venha vazio
        ...
        // 📅 2. Total de Jogos (Independente de status, se existiu reserva no mês)
        ...
        // 🚫 3. Faltas (Busca por Status e por Log de Transação)
        $reservasNoShowCount = Reserva::whereMonth('date', $mes)
            ->whereYear('date', $ano)
            ->whereIn('status', [Reserva::STATUS_NO_SHOW, 'canceled'])
            ->when($arenaId, fn($q) => $q->where('arena_id', $arenaId))
            ->count();

        // Busca específica pelo texto que vimos no seu log "Falta marcada" ou "No-Show"
        $transacoesMultaCount = FinancialTransaction::whereMonth('paid_at', $mes)
            ->whereYear('paid_at', $ano)
            ->where(function ($q) {
                $q->where('description', 'like', '%Falta marcada%')
                    ->orWhere('description', 'like', '%No-Show%')
                    ->orWhere('description', 'like', '%Multa%');
            })
            ->when($arenaId, fn($q) => $q->where('arena_id', $arenaId))
            ->distinct('reserva_id') // Evita contar sinal e quitação como duas faltas
            ->count();

        // Unifica para garantir que a View receba o dado
        $canceladasMes = max($reservasNoShowCount, $transacoesMultaCount);

        // 💸 4. Dívidas
        $totalGlobalDividas = Reserva::whereIn('status', [Reserva::STATUS_CONCLUIDA, 'completed'])
            ->whereMonth('date', $mes)
            ->whereYear('date', $ano)
        ...
        // Retorno com redundância de nomes para evitar erro na View
        return view('admin.financeiro.index', [
            'faturamentoMensal'  => $faturamentoMensal,
            'totalReservasMes'   => $totalReservasMes,
            'totalJogos'         => $totalReservasMes, // Caso a view use totalJogos
            'canceladasMes'      => $canceladasMes,
            'faltas'             => $canceladasMes, // Caso a view use faltas
