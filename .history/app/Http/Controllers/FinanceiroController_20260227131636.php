...
class FinanceiroController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // DEBUG 1: Quantas transações de "Falta" existem no banco neste mês?
        $transacoesFalta = \App\Models\FinancialTransaction::whereMonth('paid_at', $dataFiltro->month)
            ->whereYear('paid_at', $dataFiltro->year)
            ->where(function ($q) {
                $q->where('description', 'like', '%Falta%')
                    ->orWhere('description', 'like', '%No-Show%');
            })->get();

        // DEBUG 2: Quantas reservas estão como No-Show no banco?
        $reservasNoShow = \App\Models\Reserva::whereMonth('date', $dataFiltro->month)
            ->whereYear('date', $dataFiltro->year)
            ->whereIn('status', ['no_show', 'canceled'])
            ->get();

        // O COMANDO ABAIXO TRAVA A TELA E MOSTRA OS DADOS
        dd([
            'Filtro_Mes' => $dataFiltro->month,
            'Filtro_Ano' => $dataFiltro->year,
            'Contagem_Transacoes' => $transacoesFalta->count(),
            'Exemplo_Transacao' => $transacoesFalta->first() ? $transacoesFalta->first()->toArray() : 'Nenhuma encontrada',
            'Contagem_Reservas' => $reservasNoShow->count(),
            'Status_Reservas_Encontradas' => $reservasNoShow->pluck('status')->toArray()
        ]);
    }
