...
<form action="{{ route('admin.financeiro.relatorio_cortesias') }}" method="GET"
    ...
    <div class="flex flex-col gap-1">
        ...
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Nome do cliente..."
    ...
    <button type="submit" class="mt-5 bg-amber-500 hover:bg-amber-600 text-white p-2.5 rounded-xl transition-all shadow-lg shadow-amber-900/20">
        🔍
    </button>

    <a href="{{ route('admin.financeiro.relatorio_cortesias') }}" class="mt-5 bg-gray-100 dark:bg-gray-700 text-gray-500 p-2.5 rounded-xl transition-all">
        🔄
    </a>
</form>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-xl flex justify-between items-center">
                <div>
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">Total em Cortesias</p>
                    <h3 class="text-3xl font-black text-amber-600 font-mono italic">R$ {{ number_format($totalValorCortesias, 2, ',', '.') }}</h3>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-xl flex justify-between items-center">
                <div>
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">Qtd. de Vouchers</p>
                    <h3 class="text-3xl font-black text-gray-700 dark:text-white font-mono italic">{{ $cortesias->count() }} <span class="text-sm">Unid.</span></h3>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-[3rem] border border-gray-100 dark:border-gray-700 overflow-hidden shadow-2xl">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-gray-400 text-[10px] font-black uppercase tracking-widest border-b dark:border-gray-700 bg-gray-50/50 dark:bg-black/20 italic">
                        ...
                        @forelse($cortesias as $item)
                            <tr class="hover:bg-amber-50/30 dark:hover:bg-amber-900/10 transition-colors group">
                                ...
                                <td class="p-6">
                                    ...
                                    <span class="text-[9px] text-amber-600 font-bold uppercase tracking-tighter italic">Reserva #{{ $item->reserva_id }}</span>
                                </td>
                                ...
                                <td class="p-6">
                                    <div class="flex items-center gap-2">
                                        <div class="w-6 h-6 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center text-[10px] font-black text-gray-500">
                                            {{ substr($item->manager->name ?? 'S', 0, 1) }}
                                        </div>
                                        <span class="text-xs font-bold text-gray-600 dark:text-gray-400 italic">{{ $item->manager->name ?? 'Sistema' }}</span>
                                    </div>
                                </td>
                                <td class="p-6 text-right">
                                    <span class="text-amber-600 dark:text-amber-400 font-black text-xl italic font-mono tracking-tighter">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="p-24 text-center">
                                    <p class="text-gray-300 dark:text-gray-700 font-black uppercase tracking-widest italic text-3xl">Nenhuma cortesia no período</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
