            ...
            <div class="space-y-4">
                ...
                <div id="js_global_data">
                    ...
                    <input type="hidden" id="js_saldoFisicoGavetaRaw"
                        value="{{ $financialTransactions->where('payment_method', 'money')->sum('amount') }}">
                    <input type="hidden" id="js_saldoDigitalBancoRaw"
                        value="{{ $financialTransactions->where('payment_method', 'pix')->sum('amount') }}">
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    @php
                                        // 1. O valor unitário desta reserva específica
                                        $totalDestaReserva = (float) ($reserva->final_price ?? $reserva->price);

                                        // 2. Transações vinculadas a esta reserva
                                        $transacoesDaReserva = $financialTransactions->where(
                                            'reserva_id',
                                            $reserva->id,
                                        );

                                        // 3. Quanto entrou especificamente no dia selecionado (O que aparece na coluna verde)
                                        $pagoNoDia = (float) $transacoesDaReserva
                                        ...
                                        // 4. Saldo Devedor DESTA INSTÂNCIA/DIA
                                        // Se for recorrente, precisamos saber se este horário específico está pago.
                                        // Aqui tratamos o status vindo do banco que deve ser referente à data
                                        $currentStatus = $reserva->payment_status;

                                        // 5. Cálculo do Restante (Se o status for 'paid', o restante é 0, independente do histórico)
                                        $restante =
                                            $currentStatus === 'paid' ? 0 : max(0, $totalDestaReserva - $pagoNoDia);

                                        // 6. Lógica Visual de Status
                                        ...
                                        if ($reserva->status === 'no_show') {
                                            ...
                                            $statusLabel = $pagoNoDia > 0 ? 'PARCIAL' : 'PENDENTE';
                                        }
                                        ...
                                        $canPay =
                                            $restante > 0.01 &&
                                            !in_array($reserva->status, ['canceled', 'rejected', 'no_show']);
                                    @endphp

                                    <tr class="{{ $rowHighlight }} transition duration-150">
                                        <td class="px-4 py-4 text-sm font-bold text-gray-700 dark:text-gray-300">
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="text-sm font-medium text-gray-900 dark:text-white">
                                                {{ $reserva->client_name }} <span
                                                    class="text-[10px] text-gray-400">#{{ $reserva->id }}</span>
                                            </div>
                                            <div class="mt-1">
                                                <span
                                                    class="text-[9px] px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600 font-black border border-indigo-100 uppercase">
                                        ...
                                        <td class="px-4 py-4">
                                            <span
                                                class="px-2 py-0.5 text-[10px] font-bold rounded-full {{ $statusClass }}">
                                                {{ $statusLabel }}
                                            </span>
                                        </td>

                                        <td
                                            class="px-4 py-4 text-xs font-semibold {{ $reserva->is_recurrent ? 'text-fuchsia-600' : 'text-blue-600' }}">
                                            {{ $reserva->is_recurrent ? 'Recorrente' : 'Pontual' }}
                                        </td>

                                        <td
                                            class="px-4 py-4 text-right text-sm font-bold text-gray-900 dark:text-white">
                                            R$ {{ number_format($totalDestaReserva, 2, ',', '.') }}
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-center space-x-1">
                                            @if ($canPay)
                                                <button
                                                    onclick="openPaymentModal({{ $reserva->id }}, {{ $totalDestaReserva }}, {{ $restante }}, {{ $pagoNoDia }}, '{{ addslashes($reserva->client_name) }}', {{ $reserva->is_recurrent ? 'true' : 'false' }})"
                                                    class="bg-green-600 text-white text-[10px] font-bold px-2 py-1 rounded">
                                                    $ BAIXAR
                                                </button>
                                            @endif
                                            {{-- Outros botões como P. Depois e Falta... --}}
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
