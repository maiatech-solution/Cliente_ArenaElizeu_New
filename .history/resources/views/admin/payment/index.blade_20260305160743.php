                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    @php
                                        // 1. Valor unitário deste horário
                                        $valorUnitario = (float) ($reserva->final_price ?? $reserva->price);

                                        // 2. BUSCA DINÂMICA: Quanto ela pagou EXATAMENTE na data que você está visualizando?
                                        // Filtramos as transações pelo reserva_id E pela data do paid_at
                                        ...
                                        // 3. Status vindo do banco (deve ser relativo à ocorrência do dia)
                                        $currentStatus = $reserva->payment_status;

                                        // 4. LÓGICA DE QUITAÇÃO DO DIA (O segredo do Debug)
                                        // Se o status for 'paid', consideramos quitado.
                                        // Se não, calculamos o restante baseando-se APENAS no que entrou HOJE.
                                        ...
                                        if ($reserva->status === 'no_show') {
                                            ...
                                            $statusLabel = 'PAGO';
                                        } elseif ($pagoNoDia > 0 && $pagoNoDia < $valorUnitario) {
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                        ...
                                        <td class="px-4 py-4 text-right">
                                            ...
                                            {{-- Debug: Mostra o total histórico bem pequeno se houver diferença --}}
                                            @if ($reserva->total_paid > $pagoNoDia)
                                                <div class="text-[8px] text-gray-400">Histórico: R$
                                                    {{ number_format($reserva->total_paid, 2, ',', '.') }}</div>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-right text-sm font-bold">
                                            ...
                                            @else
                                                <span class="text-green-500 font-bold">QUITADO</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-center space-x-1">
                                            @if ($restanteNoDia > 0)
                                                <button
                                                    onclick="openPaymentModal({{ $reserva->id }}, {{ $valorUnitario }}, {{ $restanteNoDia }}, {{ $pagoNoDia }}, '{{ addslashes($reserva->client_name) }}')"
