                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    @php
                                        // 1. Definição do valor que deve ser pago nesta data específica
                                        $valorDoHorario = (float) ($reserva->final_price ?? $reserva->price);

                                        // 2. CÁLCULO REAL: Soma apenas as transações que ocorreram na data selecionada
                                        // Isso evita que pagamentos da semana passada somem no caixa de hoje
                                        ...
                                        // 3. Verificação de Status do Banco vs Matemática
                                        $statusBanco = $reserva->payment_status;

                                        // O restante é o preço menos o que entrou hoje (mínimo zero)
                                        $restanteNoDia = max(0, $valorDoHorario - $pagoNoDia);

                                        // 4. Lógica de exibição de Status (Prioriza a matemática do dia)
                                        if ($reserva->status === 'no_show') {
                                            $statusClass = 'bg-red-500 text-white';
                                            $statusLabel = 'FALTA';
                                        } elseif ($statusBanco === 'paid' || $restanteNoDia <= 0.01) {
                                            $statusClass = 'bg-green-100 text-green-800';
                                            $statusLabel = 'PAGO';
                                            $restanteNoDia = 0; // Garante que não mostre centavos negativos
                                        } elseif ($pagoNoDia > 0) {
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                                        ...
                                        <td class="px-4 py-4 text-right whitespace-nowrap">
                                            ...
                                            {{-- Alerta de poeira no banco: Se o acumulado total for diferente do pago hoje --}}
                                            @if (abs($reserva->total_paid - $pagoNoDia) > 0.01)
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap text-center text-sm space-x-1">
                                            @if ($restanteNoDia > 0.01)
                                                <button type="button"
                                                    onclick="openPaymentModal({{ $reserva->id }}, {{ $valorDoHorario }}, {{ $restanteNoDia }}, {{ $pagoNoDia }}, '{{ addslashes($reserva->client_name) }}')"
