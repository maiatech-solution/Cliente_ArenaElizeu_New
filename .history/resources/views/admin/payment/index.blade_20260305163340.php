                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    @php
                                        // 1. Preço total esperado para este horário
                                        $valorDoHorario = (float) ($reserva->final_price ?? $reserva->price);

                                        // 2. DINHEIRO HOJE: Soma apenas transações da DATA SELECIONADA (para conferência de caixa)
                                        ...
                                        // 3. SALDO REAL (Global): Soma todas as transações da reserva (Pagamentos - Estornos)
                                        // Se você devolveu 50 ontem (-50) e recebeu 50 hoje (+50), o saldo global é 0.
                                        $saldoGlobal = (float) $reserva->transactions()->sum('amount');

                                        // 4. CÁLCULO DO RESTANTE: Baseado no Saldo Global
                                        // Se o saldo global é 0, o restante é o valor total (100).
                                        $restanteNoDia = max(0, $valorDoHorario - $saldoGlobal);
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                                        ...
                                        <td class="px-4 py-4 text-right whitespace-nowrap">
                                            ...
                                            {{-- Info de Auditoria: Mostra o saldo global se for diferente do pago hoje --}}
                                            @if (abs($saldoGlobal - $pagoNoDia) > 0.01)
                                                <div class="text-[9px] text-gray-400 italic">
                                                    Saldo Reserva: R$ {{ number_format($saldoGlobal, 2, ',', '.') }}
                                                </div>
                                            @endif
                                        </td>

                                        {{-- Restante (Saldo Devedor Real) --}}
