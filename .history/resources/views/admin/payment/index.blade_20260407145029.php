        ...
        if (!window.__CAIXA_SCRIPT_LOADED) {
            ...
            function setupAjaxForm(formId, btnId, spinnerId, errorId, urlTemplate) {
                ...
                form.onsubmit = function(e) {
                        ...
                        fetch(targetUrl, {
                            ...
                            .then(json => {
                                ...
                                if (json.success) {
                                    ...
                                    // 🔓 LIBERAÇÃO: Só destrava se NÃO for sucesso, permitindo corrigir erro
                    ...
                    // Ações que o colaborador pode fazer SEM senha de supervisor
                    const acoesAutonomas = ['openCashForm', 'closeCashForm'];

                    // Ações que EXIGEM senha de supervisor para colaboradores
                    const acoesRestritas = ['debtForm', 'noShowForm', 'transactionForm', 'paymentForm'];

                    if (userRole === 'colaborador') {
                        if (acoesAutonomas.includes(formId)) {
                            // Abre/Fecha caixa: Envia direto
                            enviarParaOServidor();
                        } else if (acoesRestritas.includes(formId)) {
                            ...
                            // Qualquer outro formulário não mapeado envia por padrão
                            enviarParaOServidor();
                        }
                    } else {
                        // Admin ou outros cargos enviam tudo sem restrição de token
                        enviarParaOServidor();
                    }
