<x-app-layout>
    ...
    <script>
        ...
        // Captura o cargo do usuário para a trava de segurança
        ...
        // --- FUNÇÃO AJAX COM INTERCEPTAÇÃO DE SEGURANÇA ---
        async function executeAjax(url, method, data, button) {
            const originalText = button.textContent;

            // 🛡️ BLOQUEIO PARA COLABORADOR
            if (userRole === 'colaborador') {
                window.requisitarAutorizacao(token => {
                    if (token) {
                        // Se autorizado pelo gestor, prossegue para o envio
                        proceedWithRequest(url, method, data, button, originalText);
                    } else {
                        // Se cancelado ou falhou, não faz nada e mantém o botão clicável
                        console.log("Ação cancelada pelo supervisor.");
                    }
                });
            } else {
                // Admin/Gestor seguem direto
                proceedWithRequest(url, method, data, button, originalText);
            }
        }

        // Lógica real de envio (separada para ser chamada após autorização)
