                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Quadra</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Data/Horário</th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($reservas as $reserva)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <span class="px-2 py-1 text-xs font-bold rounded-md bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200 border border-indigo-200">
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap text-sm font-bold">
                                            ...
                                            @if (\App\Http\Controllers\FinanceiroController::isCashClosed($reserva->date->toDateString()))
                                                <div class="mt-1 flex items-center text-[10px] text-red-600 dark:text-red-400 font-black uppercase tracking-wider">
                                            ...
                                            @if ($sameTimeReservasCount > 0)
                                                <div class="mt-1 text-[10px] text-red-600 font-bold flex items-center italic">
                                                    <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z"></path></svg>
                                                    +{{ $sameTimeReservasCount }} interessado(s)
                                                </div>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <div class="flex flex-col space-y-2 items-center">
                                                @php
                                                    $cashIsClosed = \App\Http\Controllers\FinanceiroController::isCashClosed($reserva->date->toDateString());
                                                    $isColaborador = auth()->user()->role === 'colaborador';
                                                @endphp

                                                @if ($cashIsClosed)
                                                    <button type="button" disabled class="w-full bg-gray-400 dark:bg-gray-600 text-white px-3 py-1.5 rounded-md text-xs font-bold shadow cursor-not-allowed flex items-center justify-center opacity-60">
                                                        Bloqueado
                                                    </button>
                                                @else
                                                    <button type="button"
                                                        onclick="openConfirmModal('{{ $reserva->id }}', '{{ $reserva->client_name }}', '{{ \Carbon\Carbon::parse($reserva->date)->format('d/m/Y') }} às {{ \Carbon\Carbon::parse($reserva->start_time)->format('H:i') }}', '{{ $reserva->price }}', '{{ $reserva->arena_id }}', '{{ $reserva->start_time }}')"
                                                        class="w-full bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-md text-xs font-bold transition flex items-center justify-center shadow uppercase tracking-widest">
                                                        Confirmar
                                                    </button>
                                                @endif

                                                {{-- 🛡️ BOTÃO REJEITAR COM PROTEÇÃO DE HIERARQUIA --}}
                                                <form id="rejeitar-form-{{ $reserva->id }}" action="{{ route('admin.reservas.rejeitar', $reserva->id) }}" method="POST" class="w-full">
                                                    @csrf @method('PATCH')
                                                    <input type="hidden" name="rejection_reason" value="Rejeitada pela administração - Horário indisponível ou selecionado outro cliente.">
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        @endif
                                                        class="w-full bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-md text-xs font-bold transition shadow uppercase tracking-widest">
                                                        Rejeitar
                                                    </button>
                                                </form>
                                            </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-4 py-12 text-center text-gray-500 dark:text-gray-400 italic font-bold uppercase tracking-widest bg-gray-50 dark:bg-gray-900/50">
