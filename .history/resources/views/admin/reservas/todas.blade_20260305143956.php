<x-app-layout>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @forelse ($reservas as $reserva)
                                <tr class="odd:bg-white even:bg-gray-50 hover:bg-indigo-50 transition duration-150">
                                    ...
                                    {{-- 8. AÇÕES REVISADAS (Com Trava de Segurança para Colaborador) --}}
                                    <td class="px-4 py-3 text-sm font-medium min-w-[120px]">
                                        <div class="flex flex-col space-y-1.5">
                                            ...
                                            @php
                                                // 🛡️ Definição da Trava de Segurança
                                                $isColaborador = auth()->user()->role === 'colaborador';
                                                $temDinheiroEnvolvido = $reserva->total_paid > 0;
                                                $bloquearAcao = $isColaborador && $temDinheiroEnvolvido;
                                            ...
                                            {{-- 💰 AÇÕES FINANCEIRAS SENSÍVEIS --}}
                                            @if (!$reserva->is_fixed && in_array($reserva->status, ['confirmed', 'pending', 'concluida', 'completed']))
                                                {{-- AJUSTAR VALOR: Bloqueia se for colaborador e já estiver pago --}}
                                                <button
                                                    onclick="{{ $bloquearAcao ? "confirmarComGestor('ajuste de preço', $reserva->id)" : "openPriceUpdateModal($reserva->id, " . ($reserva->price ?? 0) . ", '$displayClient', " . ($reserva->is_recurrent ? 'true' : 'false') . ')' }}"
                                                    class="{{ $bloquearAcao ? 'bg-gray-400' : 'bg-blue-500 hover:bg-blue-600' }} text-white px-3 py-1 text-[10px] font-bold rounded shadow uppercase transition">
                                                    @if ($bloquearAcao)
                                                        <i class="fas fa-lock mr-1"></i>
                                                    @endif Ajustar Valor
                                                </button>

                                                {{-- CANCELAR DIA: Bloqueia se for colaborador e já estiver pago --}}
                                                <button
                                                    onclick="{{ $bloquearAcao ? "confirmarComGestor('cancelamento com estorno', $reserva->id)" : "openCancellationModal($reserva->id, 'PATCH', CANCEL_PONTUAL_URL, 'Deseja cancelar esta reserva?', 'Cancelar Dia', " . ($reserva->total_paid > 0 ? 'true' : 'false') . ', ' . ($reserva->total_paid ?? 0) . ')' }}"
                                                    class="{{ $bloquearAcao ? 'bg-gray-400' : 'bg-red-500 hover:bg-red-600' }} text-white px-3 py-1 text-[10px] font-bold rounded shadow uppercase transition">
                                                    @if ($bloquearAcao)
                                                        <i class="fas fa-lock mr-1"></i>
                                                    @endif Cancelar Dia
                                                </button>
                                                ...
                                                @if ($reserva->is_recurrent)
                                                    {{-- CANCELAR SÉRIE: Sempre exige gestor para colaborador (Ação Crítica) --}}
                                                    <button
                                                        onclick="{{ $isColaborador ? "confirmarComGestor('encerrar mensalista', $reserva->id)" : "openCancellationModal($reserva->id, 'DELETE', CANCEL_SERIE_URL, '🚨 Cancelar TODA a série futura?', 'Encerrar Mensalista', " . ($reserva->total_paid > 0 ? 'true' : 'false') . ', ' . ($reserva->total_paid ?? 0) . ')' }}"
                                                        class="{{ $isColaborador ? 'bg-gray-600' : 'bg-red-800 hover:bg-red-900' }} text-white px-3 py-1 text-[10px] font-bold rounded shadow uppercase transition">
                                                        Encerrar Mensalista
                                                    </button>
                                                @endif

                                                {{-- NO-SHOW: Colaborador pode marcar, pois não envolve devolver dinheiro, apenas registrar falta --}}
                                                <button
                                                    onclick="openNoShowModal({{ $reserva->id }}, 'Registrar Falta', 'O cliente não apareceu?', REGISTER_NOSHOW_URL)"
                                            ...
                                            @if (in_array($reserva->status, ['cancelled', 'rejected', 'no_show']))
                                                <button
                                                    onclick="openReactivationModal({{ $reserva->id }}, 'Reativar Agendamento', 'Deseja reativar?', REACTIVATE_URL)"
    ...
    <script>
        ...
        /**
         * Ponte de Segurança para o Colaborador
         * @param {string} acao - Nome da ação para o alerta
         * @param {int} id - ID da reserva
         */
        function confirmarComGestor(acao, id) {
            // 1. Chamamos o modal de autenticação que você já possui no sistema
            // Se o seu método de abrir o modal for diferente, ajuste o nome da função abaixo
            if (typeof abrirModalAutenticacaoAdmin === "function") {
                abrirModalAutenticacaoAdmin(function(tokenSupervisor) {
                    // Se cair aqui, significa que o gestor digitou a senha correta
                    console.log("Autorização concedida para: " + acao);

                    // 2. Agora liberamos o modal real que o colaborador queria abrir
                    // Buscamos os dados da linha da tabela para preencher o modal
                    if (acao === 'ajuste de preço') {
                        // Aqui você pode disparar a função original de preço
                        // Você precisará capturar os valores da linha ou passar via parâmetros extras
                        alert("Acesso Liberado. O gestor autorizou o ajuste de preço para a reserva #" + id);
                        // Exemplo: openPriceUpdateModal(id, valor_atual, nome_cliente, recorrente);
                    } else if (acao === 'cancelamento com estorno') {
                        alert("Acesso Liberado. O gestor autorizou o cancelamento da reserva #" + id);
                        // Exemplo: openCancellationModal(id, 'PATCH', ...);
                    }
                });
            } else {
                // Fallback caso a função de autenticação mude de nome
                alert("🚨 Erro de Sistema: Módulo de autorização não encontrado. Contate o suporte.");
            }
        }
    </script>
</x-app-layout>
