                                ...
                                <tr class="odd:bg-white even:bg-gray-50 hover:bg-indigo-50 transition duration-150">
                                    ...
                                    <td class="px-4 py-3 text-sm font-medium min-w-[120px]">
                                        <div class="flex flex-col space-y-1.5">
                                            ...
                                            {{-- Manutenção com Trava --}}
                                            @if ($reserva->status === 'maintenance')
                                                <button
                                                    onclick="checkAuthAndRun(() => handleFixedSlotToggle({{ $reserva->id }}, 'free'))"
                                            ...
                                            @else
                                                <button
                                                    onclick="checkAuthAndRun(() => handleFixedSlotToggle({{ $reserva->id }}, 'maintenance', {{ $reserva->client_name || $reserva->user_id ? 'true' : 'false' }}, '{{ addslashes($reserva->client_name ?? 'Externo') }}'))"
                                            ...
                                            {{-- Ajustar Valor com Trava (Apenas se não for fixo e estiver ativa) --}}
                                            @if (!$reserva->is_fixed && in_array($reserva->status, ['confirmed', 'pending']))
                                                <button
                                                    onclick="checkAuthAndRun(() => openPriceUpdateModal({{ $reserva->id }}, {{ $reserva->price ?? 0 }}, '{{ addslashes($reserva->client_name ?? 'Cliente') }}', {{ $reserva->is_recurrent ? 'true' : 'false' }}))"
