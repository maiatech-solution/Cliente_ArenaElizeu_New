       ...
       {{-- -------------------------------------------------------------------------------- --}}
                    ...
                    <form action="{{ route('bar.cash.open') }}" method="POST" id="formOpenCash">
                        ...
                        <div class="mb-6 p-4 bg-gray-800/50 border border-gray-800 rounded-3xl text-center">
                            @if (in_array(auth()->user()->role, ['admin', 'gestor']))
                                <span class="text-[9px] font-black text-orange-500 uppercase block mb-2 tracking-widest">Confirme sua Senha</span>
                            @else
                                <span class="text-[9px] font-black text-orange-500 uppercase block mb-2 tracking-widest animate-pulse">🔒 Senha do Gestor Necessária</span>
        ...
        @elseif(!$currentSession)
            ...
            <div class="py-20 text-center opacity-20">
                <p class="text-gray-600 font-black uppercase tracking-widest italic text-3xl">Nenhum registro nesta data</p>
            </div>
        @else
            {{-- TELA: CAIXA EM OPERAÇÃO (CARDS + TABELA) --}}
            <div class="grid grid-cols-1 md:grid-cols-5 gap-6 mb-10">
                <div class="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 relative shadow-2xl border-l-4 border-l-emerald-500">
                    <span class="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-2 italic">Dinheiro em Gaveta</span>
                    <span class="text-4xl font-black text-white italic tracking-tighter font-mono">R$ {{ number_format($dinheiroGeral ?? 0, 2, ',', '.') }}</span>
                </div>
                <div class="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 shadow-2xl border-l-4 border-l-cyan-400">
                    <span class="text-[10px] font-black text-cyan-400 uppercase tracking-widest block mb-2 italic">Total Digital</span>
                    <span class="text-4xl font-black text-white italic tracking-tighter font-mono">R$ {{ number_format($faturamentoDigital ?? 0, 2, ',', '.') }}</span>
                </div>
                <div class="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 shadow-2xl border-l-4 border-l-red-500">
                    <span class="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-2 italic">Sangrias / Saídas</span>
                    <span class="text-4xl font-black text-white italic tracking-tighter font-mono">R$ {{ number_format($sangrias ?? 0, 2, ',', '.') }}</span>
                </div>
                <div class="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 shadow-2xl border-l-4 {{ ($totalEstornado ?? 0) > 0 ? 'border-l-orange-600' : 'border-l-gray-700 opacity-50' }}">
                    <span class="text-[10px] font-black {{ ($totalEstornado ?? 0) > 0 ? 'text-orange-500' : 'text-gray-500' }} uppercase tracking-widest block mb-2 italic">Total Estornado</span>
                    <span class="text-4xl font-black {{ ($totalEstornado ?? 0) > 0 ? 'text-white' : 'text-gray-600' }} italic tracking-tighter font-mono">R$ {{ number_format($totalEstornado ?? 0, 2, ',', '.') }}</span>
                </div>
                <div class="bg-gray-800/50 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl border-l-4 border-l-white/20">
                    <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2 italic">Faturamento Real</span>
                    <span class="text-4xl font-black text-white italic tracking-tighter font-mono">R$ {{ number_format($totalBruto ?? 0, 2, ',', '.') }}</span>
                </div>
            </div>
            ...
            <div class="bg-gray-900 rounded-[3rem] border border-gray-800 overflow-hidden shadow-2xl">
                <div class="p-8 border-b border-gray-800 flex justify-between items-center bg-gray-800/20">
                    ...
                    <div class="flex items-center gap-4">
                        <span class="text-[10px] text-gray-600 font-bold uppercase tracking-tighter italic font-black underline decoration-green-500/30 underline-offset-4">Faturado: R$ {{ number_format($totalBruto ?? 0, 2, ',', '.') }}</span>
                        <span class="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Sessão ID: #{{ $currentSession->id }}</span>
                        <span class="{{ $currentSession->status == 'open' ? 'text-green-500 animate-pulse' : 'text-red-500' }} text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                            <span class="w-2 h-2 {{ $currentSession->status == 'open' ? 'bg-green-500' : 'bg-red-500' }} rounded-full"></span>
                        ...
                        <tbody class="divide-y divide-gray-800/50">
                            @forelse($movements as $mov)
                                @php
                                    ...
                                    $corValor = $isSaida ? 'text-red-500' : ($isVenda ? 'text-green-500' : 'text-white');
                                    $metodo = strtolower($mov->payment_method);
                                    $bgMetodo = $metodo == 'dinheiro' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : (in_array($metodo, ['pix', 'transferencia']) ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20' : 'bg-purple-500/10 text-purple-400 border-purple-500/20');
                                ...
                                <tr class="hover:bg-white/[0.02] transition-colors group">
                                    ...
                                    <td class="p-6">
                                        <span class="text-white block font-black text-xs uppercase tracking-tight">{{ $tituloDescricao }}</span>
                                        @if ($motivoTexto)<span class="text-[10px] text-orange-400 font-bold italic block mt-1">💬 Motivo: {{ $motivoTexto }}</span>@endif
                                        @if ($autorizadorNome)<span class="text-[9px] text-indigo-400 font-black uppercase tracking-widest block mt-1">🔐 Autorizado por: {{ $autorizadorNome }}</span>@endif
                                        <div class="flex items-center gap-2 mt-2">
                                            <span class="text-[8px] uppercase font-black px-2 py-0.5 rounded border {{ $isSaida ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-blue-500/10 text-blue-500 border-blue-500/20' }}">{{ $mov->type }}</span>
                                            @if ($mov->payment_method)<span class="text-[8px] uppercase font-black px-2 py-0.5 rounded border {{ $bgMetodo }}">💳 {{ $mov->payment_method }}</span>@endif
                                        </div>
                                    </td>
                                    <td class="p-6">
                                        <div class="flex items-center gap-2">
                                            <div class="w-6 h-6 rounded-lg bg-gray-800 border border-gray-700 flex items-center justify-center text-[10px] text-orange-500 font-black">{{ substr($mov->user->name, 0, 1) }}</div>
                                            <span class="text-gray-400 text-[10px] font-bold uppercase italic tracking-widest">{{ $mov->user->name }}</span>
                                        </div>
                                    </td>
                                    <td class="p-6 text-right font-black italic text-xl {{ $corValor }}">{{ $isSaida ? '-' : ($isVenda ? '+' : '') }} R$ {{ number_format($mov->amount, 2, ',', '.') }}</td>
                                </tr>
                            @empty
        ...
        {{-- -------------------------------------------------------------------------------- --}}
        {{-- 🔍 SEÇÃO DE AUDITORIA: TURNOS ENCERRADOS (APARECE SEMPRE ABAIXO) --}}
        {{-- -------------------------------------------------------------------------------- --}}
        @if (in_array(auth()->user()->role, ['admin', 'gestor']))
            <div class="mb-10 animate-in mt-12 fade-in duration-700">
                <div class="flex items-center gap-4 mb-6">
                    <h3 class="text-[10px] font-black text-orange-500 uppercase tracking-[0.4em] italic">Turnos Encerrados (Auditoria)</h3>
                        ...
                        <div class="bg-gray-900/50 border-2 border-gray-800 p-6 rounded-[2.5rem] hover:border-orange-500/30 transition-all group relative overflow-hidden">
                            ...
                            <div class="space-y-3 mb-6 bg-black/20 p-5 rounded-3xl border border-white/5">
                                <div class="flex justify-between items-center">
                                    <p class="text-[8px] text-gray-600 font-black uppercase italic tracking-widest">Faturamento Real</p>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="text-[11px] text-gray-400 font-bold tracking-tighter">{{ \Carbon\Carbon::parse($sessao->opened_at)->format('H:i') }}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-[8px] text-gray-600 font-black uppercase italic">Fechamento</p>
                                        <p class="text-[11px] text-gray-400 font-bold tracking-tighter">{{ \Carbon\Carbon::parse($sessao->closed_at)->format('H:i') }}</p>
                                    </div>
                                </div>
                            </div>
