            ...
            <div class="space-y-4 mb-8">
                ...
                <div class="bg-black/40 p-6 rounded-3xl border border-gray-800 shadow-inner space-y-3">
                    <span
                        ...
                        🧮 Composição da Gaveta (Dinheiro)
                    </span>

                    <div class="flex justify-between text-xs">
                        <span class="text-gray-400 uppercase font-bold italic">Troco Inicial (Abertura):</span>
                        <span class="text-white font-mono">R$
                            {{ number_format($currentSession->opening_balance ?? 0, 2, ',', '.') }}</span>
                    </div>

                    <div class="flex justify-between text-xs">
                        <span class="text-gray-400 uppercase font-bold italic">(+) Vendas em Espécie:</span>
                        <span class="text-white font-mono">R$
                            {{ number_format($dinheiroGeral ?? 0, 2, ',', '.') }}</span>
                    </div>

                    <div class="flex justify-between text-xs">
                        <span class="text-gray-400 uppercase font-bold italic">(+) Reforços:</span>
                        <span class="text-green-500 font-mono font-bold">+ R$
                            {{ number_format($totalReforcos ?? 0, 2, ',', '.') }}</span>
                    </div>

                    <div class="flex justify-between text-xs border-b border-gray-800 pb-2">
                        <span class="text-gray-400 uppercase font-bold italic">(-) Sangrias:</span>
                        <span class="text-red-500 font-mono font-bold">- R$
                            {{ number_format($totalSangrias ?? 0, 2, ',', '.') }}</span>
                    </div>

                    <div class="flex justify-between items-center pt-2">
                        <div>
                            <span
                                class="text-[9px] font-black text-emerald-500 uppercase block tracking-widest leading-tight">
                                Esperado em Espécie<br>(Gaveta Física)
                            </span>
                        </div>
                        <span class="text-white font-black text-3xl italic font-mono tracking-tighter">
                            {{-- Fundo + Vendas Dinheiro + Reforços - Sangrias --}}
                            R$
                            {{ number_format(($currentSession->opening_balance ?? 0) + ($dinheiroGeral ?? 0) + ($totalReforcos ?? 0) - ($totalSangrias ?? 0), 2, ',', '.') }}
                        </span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="bg-orange-600/10 p-5 rounded-3xl border border-orange-600/20">
                        <span
                            ...
                            Faturamento Líquido<br>(Vendas do Turno)
                        </span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="text-gray-500 uppercase text-[10px] font-black ml-2 mb-2 block tracking-widest">
                            Dinheiro contado na Gaveta (Físico)
                        </label>
...
<script>
     ...
     * Agora considera: Saldo Inicial + Vendas Cash + Reforços - Sangrias
     */
    function calcularDiferenca() {
        ...
        const vendasDinheiro = parseFloat("{{ $dinheiroGeral ?? 0 }}");
        const reforcos = parseFloat("{{ $totalReforcos ?? 0 }}");
        const sangrias = parseFloat("{{ $totalSangrias ?? 0 }}");

        // O valor que o sistema espera que o Marlon tenha na mão (Dinheiro físico)
        ...
        if (input.value === "" || input.value === "0") {
            display.innerText = "CONTE O DINHEIRO FÍSICO DA GAVETA (R$ " + totalEsperadoNaGaveta.toLocaleString(
            'pt-br', {
        ...
        // Diferença: O que ele digitou menos o que o sistema calculou para a gaveta
