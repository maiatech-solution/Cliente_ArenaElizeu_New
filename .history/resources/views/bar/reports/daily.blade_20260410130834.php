<x-bar-layout>
    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div
            ...
            <h2 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-10 italic">Performance de Vendas
                Diárias</h2>
            ...
            <div
                ...
                @php
                    // Garante que o teto do gráfico seja baseado no maior valor real encontrado
                    $maxChart = $maxValorDia > 0 ? $maxValorDia : 1;
                @endphp

                @foreach ($datas as $dia => $valores)
                    @php
                        // 🎯 Sincronização de faturamento: Mesas + PDV
                        $faturamentoMesas = (float) ($valores['mesas'] ?? 0);
                        $faturamentoPdv = (float) ($valores['pdv'] ?? 0);
                        $totalDia = $faturamentoMesas + $faturamentoPdv;

                        $carbonDia = \Carbon\Carbon::parse($dia);
                        $altura = ($totalDia / $maxChart) * 100;
                    @endphp

                    {{-- Filtra apenas os dias do mês selecionado --}}
                    @if ($carbonDia->format('m') == $mesAtualFiltro)
                        <div class="flex-1 min-w-[35px] flex flex-col items-center group relative">

                            {{-- Tooltip: Só mostra se o valor for > 0 para não poluir o layout --}}
                            <div
                                class="absolute -top-10 bg-orange-600 text-white text-[10px] font-black px-3 py-1.5 rounded-lg shadow-2xl opacity-0 group-hover:opacity-100 transition-all z-20 whitespace-nowrap pointer-events-none">
                    ...
                    {{ $carbonDia->isToday() ? 'bg-orange-500 shadow-[0_0_25px_rgba(234,88,12,0.5)]' : ($totalDia > 0 ? 'bg-gray-700 group-hover:bg-orange-600' : 'bg-gray-800/20') }}"
                                ...
                                {{-- Efeito de brilho no topo da barra se houver venda --}}
                                @if ($totalDia > 0)
                                    <div class="absolute top-0 left-0 w-full h-[2px] bg-white/20 rounded-t-lg"></div>
                                @endif
                            </div>
                            ...
                            <span class="text-[6px] font-black uppercase text-gray-700 tracking-tighter italic">
