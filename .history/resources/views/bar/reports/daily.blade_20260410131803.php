    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div
            ...
            <div
                ...
                @php
                    // Garante que o teto do gráfico seja o recorde real
                    $maxChart = $maxValorDia > 0 ? $maxValorDia : 1;
                @endphp

                @foreach ($datas as $dia => $valores)
                    @php
                        // 🎯 Cálculo exato do faturamento do dia
                        $totalDia = (float) ($valores['mesas'] ?? 0) + (float) ($valores['pdv'] ?? 0);

                        // Forçamos a criação da data a partir da chave do array
                        $carbonDia = \Carbon\Carbon::parse($dia);
                        $altura = ($totalDia / $maxChart) * 100;
                    @endphp

                    {{-- Filtramos apenas os dias que pertencem ao mês de referência --}}
                    @if ($carbonDia->format('m') == $mesAtualFiltro)
                        <div class="flex-1 min-w-[35px] flex flex-col items-center group relative">

                            {{-- 🎈 Tooltip (Balaõzinho ao passar o mouse) --}}
                            <div
