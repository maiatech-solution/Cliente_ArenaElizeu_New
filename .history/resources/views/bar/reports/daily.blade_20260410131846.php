<x-bar-layout>
    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {{-- 📈 GRÁFICO DE BARRAS COM DEBUG ATIVO E CORREÇÃO DE RENDERIZAÇÃO --}}
        <div
            ...
            {{-- Container do Gráfico: h-64 garante que as barras tenham onde crescer --}}
            <div
                ...
                @php
                    // Recálculo de segurança para garantir que a divisão nunca seja por zero
                    $maxSeguranca =
                        collect($datas)
                            ->map(function ($d) {
                ...
                @foreach ($datas as $dia => $valores)
                    ...
                    @if ($carbonDia->format('m') == $mesAtualFiltro)
                        <div class="flex-1 min-w-[35px] flex flex-col items-center group relative">
                            ...
                            {{-- 📊 A BARRA (Ajustada com Laranja para teste de visibilidade) --}}
                            <div class="w-full max-w-[22px] rounded-t-lg transition-all duration-700 relative z-10
                        {{ $carbonDia->isToday() ? 'bg-orange-500 shadow-[0_0_25px_rgba(234,88,12,0.5)]' : ($totalDia > 0 ? 'bg-orange-600/80 group-hover:bg-orange-500' : 'bg-gray-800/20') }}"
                                style="height: {{ max($altura, 3) }}%; min-height: {{ $totalDia > 0 ? '4px' : '2px' }};">

                                {{-- Brilho no topo da barra --}}
                                @if ($totalDia > 0)
                                    <div class="absolute top-0 left-0 w-full h-[2px] bg-white/40 rounded-t-lg"></div>
                                @endif
                            </div>
