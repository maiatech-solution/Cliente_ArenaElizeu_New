<x-bar-layout>
    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {{-- 📈 GRÁFICO DE PERFORMANCE DIÁRIA (VENDAS + LUCRO) --}}
        <div
            class="bg-gray-900 border border-gray-800 rounded-[3rem] p-6 mb-10 shadow-2xl relative overflow-hidden text-white italic">
            <div class="flex justify-between items-center mb-10">
                <div>
                    <h2 class="text-xs font-black text-gray-400 uppercase tracking-widest italic">Performance de Vendas
                        Diárias</h2>
                    <p class="text-[8px] text-gray-600 uppercase font-bold mt-1">Comparativo visual entre faturamento
                        bruto e margem líquida</p>
                </div>
            </div>
            ...
            <div
                ...
                @php
                    // Garante que o teto do gráfico seja o recorde do mês para escala correta
                    $maxSeguranca =
                        collect($datas)
                            ->map(function ($d) {
                ...
                @foreach ($datas as $dia => $valores)
                    @php
                        ...
                        // Altura total da barra baseada no faturamento
                        $alturaTotal = ($totalDia / $maxSeguranca) * 100;

                        // Proporção do lucro dentro da própria barra
                    ...
                    @if ($carbonDia->format('m') == $mesAtualFiltro)
                        ...
                        {{ $totalDia > 0 ? 'bg-gray-800' : 'bg-gray-800/20' }}"
                                style="height: {{ max($alturaTotal, 3) }}%; min-height: {{ $totalDia > 0 ? '4px' : '2px' }};">

                                @if ($totalDia > 0)
                                    {{-- PARTE LARANJA: Representa o Custo Operacional --}}
                                    <div
                                        class="w-full bg-orange-600/40 flex-1 relative group-hover:bg-orange-600/60 transition-colors">
                                        <div class="absolute top-0 left-0 w-full h-[1px] bg-white/20"></div>
                                    </div>

                                    {{-- PARTE VERDE: Representa o Lucro Líquido --}}
                            ...
                            {{-- Labels (Data e Dia) --}}
                            <span
                                class="mt-3 text-[9px] font-black {{ $carbonDia->isToday() ? 'text-orange-500 underline' : 'text-gray-600' }}">
                                {{ $carbonDia->format('d') }}
                            </span>
                            <span
