<x-bar-layout>
    @php
        // 1. CÁLCULOS TÉCNICOS INICIAIS (Unificados)
        $totalGeralMes = 0;
        $lucroGeralMes = 0;
        $maxValorDia = 0;
        ...
        foreach ($datas as $dia => $valores) {
            $carbonDia = \Carbon\Carbon::parse($dia)->startOfDay();
            if ($carbonDia->format('m') == $mesAtualFiltro) {
                ...
                $lucroGeralMes += $lucroDia; // Soma tudo (lucros e prejuízos)
    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {{-- 📈 GRÁFICO DE PERFORMANCE DIÁRIA (COM RESPIRO NO TOPO) --}}
        <div
            ...
            <div
                ...
                @php
                    // 🎯 O PULO DO GATO: Multiplicamos o MAX por 1.2 para criar um "respiro" de 20% no topo do gráfico
                    $maxReal =
                        collect($datas)
                            ->map(function ($d) {
                ...
                @foreach ($datas as $dia => $valores)
                    @php
                        ...
                        // Altura total da barra (agora com folga no topo)
                        $alturaTotal = ($totalDia / $maxSeguranca) * 100;

                        // Proporção do lucro dentro da barra
                    ...
                    @if ($carbonDia->format('m') == $mesAtualFiltro)
                        <div class="flex-1 min-w-[35px] flex flex-col items-center group relative">
                            ...
                            {{-- 📊 A BARRA EMPILHADA --}}
                            <div class="w-full max-w-[22px] rounded-t-lg transition-all duration-700 relative z-10 flex flex-col justify-end overflow-hidden
                        {{ $carbonDia->isToday() ? 'ring-2 ring-orange-500 ring-offset-2 ring-offset-gray-900 shadow-[0_0_20px_rgba(234,88,12,0.3)]' : '' }}
                        {{ $totalDia > 0 ? 'bg-gray-800' : 'bg-gray-800/10' }}"
                                ...
                                @if ($totalDia > 0)
                                    ...
                                    {{-- PARTE VERDE: Lucro --}}
                                    <div class="w-full bg-emerald-500 relative shadow-[0_-5px_10px_rgba(16,185,129,0.2)]"
                            ...
                            <span
                                class="text-[7px] font-black uppercase text-gray-700 tracking-tighter italic leading-none mt-1">
        ...
        <style>
            /* Estilização extra para o efeito de stack */
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-800/40 font-mono">
                        @foreach (array_reverse($datas, true) as $dia => $valores)
                            ...
                            {{-- 🎯 AJUSTE: Mostra o dia se: teve venda OU teve lucro/prejuízo OU é hoje --}}
                                ...
                                <tr
                                    ...
                                    <td class="p-8">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="w-14 h-14 rounded-2xl bg-black border border-gray-800 flex flex-col items-center justify-center text-base font-black {{ $carbon->isToday() ? 'text-orange-500 border-orange-600/30' : 'text-gray-500' }} uppercase italic shadow-inner">
                                    ...
                                    {{-- 💰 COLUNA DE LUCRO COM TRATAMENTO PARA PREJUÍZO --}}
                                    <td
                                        class="p-8 text-right {{ $lucroDia < 0 ? 'bg-red-500/[0.02]' : 'bg-emerald-500/[0.02]' }}">
                                        <span
                                            class="text-xl font-black italic tracking-tighter {{ $lucroDia < 0 ? 'text-red-500' : 'text-emerald-500' }}">
                                            R$ {{ number_format($lucroDia, 2, ',', '.') }}
                                        </span>
                                        <p
                                            class="text-[7px] font-black uppercase mt-1 {{ $lucroDia < 0 ? 'text-red-600' : 'text-emerald-600' }}">
                                            {{ $lucroDia < 0 ? 'Prejuízo Operacional' : 'Margem: ' . ($totalDia > 0 ? number_format(($lucroDia / $totalDia) * 100, 1) : 0) . '%' }}
                                        </p>
                                    </td>
