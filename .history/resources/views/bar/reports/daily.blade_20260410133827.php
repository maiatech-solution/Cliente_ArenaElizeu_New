<x-bar-layout>
    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 italic font-sans">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-[3rem] p-8 mb-10 shadow-2xl relative overflow-hidden text-white italic">
            <h2 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-10">Performance Diária</h2>
            <div class="flex items-end justify-between gap-1 h-64 px-2 border-b border-gray-800/50 pb-2 overflow-x-auto no-scrollbar font-mono">
                @php $maxSeguranca = $maxValorDia * 1.2 ?: 1; @endphp
                @foreach ($datas as $dia => $valores)
                    @php
                        $carbonDia = \Carbon\Carbon::parse($dia);
                    ...
                    @if ($carbonDia->format('m') == $mesAtualFiltro)
                        <div class="flex-1 min-w-[35px] flex flex-col items-center group relative">
                            <div class="absolute -top-16 bg-gray-950 border border-gray-800 text-white text-[9px] font-black px-3 py-2 rounded-xl shadow-2xl opacity-0 group-hover:opacity-100 transition-all z-20">
                                <div>Venda: R$ {{ number_format($totalDia, 2, ',', '.') }}</div>
                                <div class="{{ $lucroDia < 0 ? 'text-red-400' : 'text-emerald-400' }}">Lucro: R$ {{ number_format($lucroDia, 2, ',', '.') }}</div>
                            </div>
                            <div class="w-full max-w-[22px] rounded-t-lg transition-all duration-700 relative z-10 flex flex-col justify-end overflow-hidden {{ $totalDia > 0 ? 'bg-gray-800' : 'bg-gray-800/10' }}" style="height: {{ max($alturaTotal, 2) }}%;">
                                @if($totalDia > 0)
                                    <div class="w-full bg-orange-600/30 flex-1 relative"></div>
                                    <div class="w-full bg-emerald-500" style="height: {{ $porcentagemLucro }}%"></div>
                                @elseif($lucroDia < 0)
                                    <div class="w-full bg-red-600 h-full"></div>
                                @endif
                            </div>
                            <span class="mt-3 text-[9px] font-black text-gray-500">{{ $carbonDia->format('d') }}</span>
                        </div>
                    @endif
                @endforeach
            </div>
