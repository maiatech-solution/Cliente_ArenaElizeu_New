<x-bar-layout>
    @php
        ...
        $diasComAtividade = collect($datas)
            ->filter(function ($d) use ($mesAtualFiltro) {
    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 italic font-sans">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h1 class="text-4xl font-black text-white uppercase tracking-tighter italic leading-none">Resumo
                        <span class="text-orange-600">Diário</span></h1>
                    <p class="text-gray-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">Análise de
                        rentabilidade Maiatech</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-blue-500 shadow-2xl">
                ...
                <h3 class="text-3xl font-black text-white italic tracking-tighter font-mono">R$
                    {{ number_format($maxValorDia, 2, ',', '.') }}</h3>
            </div>
            <div
                ...
                <h3 class="text-3xl font-black text-white italic tracking-tighter font-mono">R$
                    {{ number_format($mediaDiaria, 2, ',', '.') }}</h3>
            </div>
            <div class="bg-gray-900 p-6 rounded-[2rem] border border-gray-800 border-l-4 border-l-gray-600 shadow-2xl">
                ...
                <h3 class="text-3xl font-black text-white italic tracking-tighter font-mono opacity-40">R$
                    {{ number_format($totalGeralMes, 2, ',', '.') }}</h3>
            </div>
            <div
                ...
                <p class="text-[10px] font-black text-emerald-100 uppercase tracking-widest mb-1 font-bold">Lucro
                    Líquido Real</p>
                <h3 class="text-3xl font-black text-white italic tracking-tighter font-mono">R$
                    {{ number_format($lucroGeralMes, 2, ',', '.') }}</h3>
                <span class="text-[8px] text-emerald-100 font-black uppercase mt-1">Margem Global:
                    {{ $totalGeralMes > 0 ? number_format(($lucroGeralMes / $totalGeralMes) * 100, 1) : 0 }}%</span>
            </div>
        </div>
...
<div class="bg-gray-900 border border-gray-800 rounded-[3rem] p-8 mb-10 shadow-2xl relative overflow-hidden text-white italic">
    ...
    <div class="flex items-end justify-between h-64 px-1 border-b border-gray-800/50 pb-2 font-mono overflow-hidden">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-[3rem] shadow-2xl overflow-hidden">
            <div
                class="p-8 border-b border-gray-800 bg-black/20 flex justify-between items-center font-mono uppercase italic text-[10px] font-black text-white">
                    ...
                    <tbody class="divide-y divide-gray-800/40">
                        @foreach (array_reverse($datas, true) as $dia => $valores)
                            @php
                                $totalDia = (float) ($valores['mesas'] ?? 0) + (float) ($valores['pdv'] ?? 0);
                                $lucroDia =
                                    (float) ($valores['lucro_mesas'] ?? 0) + (float) ($valores['lucro_pdv'] ?? 0);
                                ...
                                <tr class="hover:bg-white/[0.02] transition-colors group">
                                    <td class="p-8">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="w-14 h-14 rounded-2xl bg-black border border-gray-800 flex flex-col items-center justify-center text-base font-black {{ $carbon->isToday() ? 'text-orange-500 border-orange-600' : 'text-gray-500' }}">
                                                <span>{{ $carbon->format('d') }}</span>
                                                <span class="text-[8px]">{{ $diasSemana[$carbon->format('l')] }}</span>
                                            </div>
                                            <div>
                                                <span
                                                    class="text-white font-black block uppercase text-base">{{ $diasSemanaFull[$carbon->format('l')] }}</span>
                                                <span
                                                    class="text-[10px] text-gray-600 font-bold uppercase">{{ $carbon->format('d/m/Y') }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="p-8 text-right bg-black/10">
                                        <span
                                            class="{{ $lucroDia < 0 ? 'text-red-500' : 'text-emerald-500' }} text-xl font-black italic tracking-tighter leading-none">
                                            R$ {{ number_format($lucroDia, 2, ',', '.') }}
                                        </span>
                                        <p
                                            class="text-[7px] font-black uppercase mt-1 {{ $lucroDia < 0 ? 'text-red-700' : 'text-emerald-700' }}">
                                            {{ $lucroDia < 0 ? 'Prejuízo' : 'Margem: ' . ($totalDia > 0 ? number_format(($lucroDia / $totalDia) * 100, 1) : 0) . '%' }}
                                        </p>
                                    </td>
                                    <td class="p-8 text-right">
                                        <span
                                            class="text-3xl font-black text-white italic tracking-tighter leading-none">R$
                                            {{ number_format($totalDia, 2, ',', '.') }}</span>
                                    </td>
                                </tr>
                            @endif
                        @endforeach
