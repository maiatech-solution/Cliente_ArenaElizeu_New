<x-bar-layout>
    <div class="max-w-[1400px] mx-auto px-6 py-8">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-10">
            ...
            <form action="{{ route('bar.reports.operators') }}" method="GET"
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="p-6">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="w-10 h-10 bg-gray-800 rounded-2xl flex items-center justify-center font-black text-orange-500 border border-gray-700 shadow-inner group-hover:border-orange-500/50 transition-all">
                                            {{ substr($rank->user->name, 0, 1) }}
                                        </div>
                                        <div>
                                            <span
                                                class="text-white font-black uppercase italic text-sm block leading-none">{{ $rank->user->name }}</span>
                                            <span
                                                class="text-[8px] text-gray-600 font-bold uppercase tracking-widest">{{ $rank->user->role }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="p-6 text-center text-white font-mono font-bold">{{ $rank->qtd_vendas }}</td>
                                <td class="p-6 text-right text-gray-400 font-mono text-xs">R$
                                    {{ number_format($rank->total_bruto, 2, ',', '.') }}</td>
                                <td class="p-6 text-right text-red-500/50 font-mono text-xs">- R$
                                    {{ number_format($rank->total_estornado, 2, ',', '.') }}</td>
                                <td
                                    class="p-6 text-right text-white font-black text-2xl italic font-mono tracking-tighter">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="p-24 text-center opacity-20">
                                    <p class="text-gray-600 font-black uppercase tracking-widest italic text-3xl">Nenhum
                                        dado no período</p>
                                </td>
                            </tr>
                        @endforelse
