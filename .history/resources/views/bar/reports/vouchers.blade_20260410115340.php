<x-bar-layout>
    <div class="max-w-[1400px] mx-auto px-6 py-8 italic text-white">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-10">
            ...
            <form action="{{ route('bar.reports.vouchers') }}" method="GET"
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-[9px] font-black text-gray-600 uppercase ml-2">Pesquisar Cliente/ID</label>
                    <input type="text" name="search" value="{{ $search }}" placeholder="Ex: Alan ou #123"
                ...
                <button type="submit" class="mt-5 bg-orange-600 hover:bg-orange-500 text-white p-2.5 rounded-xl transition-all shadow-lg">
                    🔍
                </button>

                <a href="{{ route('bar.reports.vouchers') }}" class="mt-5 bg-gray-800 hover:bg-gray-700 text-gray-400 p-2.5 rounded-xl transition-all">
                    🔄
                </a>
            </form>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            <div class="bg-gray-900 p-8 rounded-[3rem] border border-gray-800 shadow-2xl flex justify-between items-center">
                <div>
                    <p class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] italic">Impacto em Cortesias</p>
                    <h3 class="text-4xl font-black text-orange-600 font-mono tracking-tighter italic">R$ {{ number_format($totalValor, 2, ',', '.') }}</h3>
                </div>
                <div class="text-4xl opacity-20 group-hover:opacity-100 transition-opacity">🎁</div>
            </div>
            <div class="bg-gray-900 p-8 rounded-[3rem] border border-gray-800 shadow-2xl flex justify-between items-center">
                <div>
                    <p class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] italic">Total de Vendas Voucher</p>
                    <h3 class="text-4xl font-black text-white font-mono tracking-tighter italic">{{ $totalQtd }} <span class="text-sm">Vendas</span></h3>
                </div>
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="p-6">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[9px] text-orange-500 font-bold uppercase">ID: #{{ $item->id }}</span>
                                    </div>
                                </td>
                                <td class="p-6">
                                    <span class="text-xs text-gray-400 font-bold uppercase">{{ $item->user->name ?? 'Sistema' }}</span>
                                </td>
