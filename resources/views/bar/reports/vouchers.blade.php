    ...
    <div x-data="{
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-10">
            ...
            <form action="{{ route('bar.reports.vouchers') }}" method="GET"
                ...
                <div class="flex flex-col gap-1">
                    <label
                        class="text-[9px] font-black text-gray-600 uppercase ml-2 italic text-center leading-none">Início</label>
                ...
                <div class="flex flex-col gap-1">
                    <label
                        class="text-[9px] font-black text-gray-600 uppercase ml-2 italic text-center leading-none">Fim</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            <div
                class="bg-gray-900 p-8 rounded-[3rem] border border-gray-800 shadow-2xl flex justify-between items-center transition-all hover:border-orange-900/50">
                <div>
                    <p class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] italic leading-none mb-2">
                        Impacto em Cortesias</p>
                    <h3 class="text-4xl font-black text-orange-600 font-mono tracking-tighter italic leading-none">R$
                        {{ number_format($totalValor, 2, ',', '.') }}</h3>
                </div>
                <div class="text-4xl opacity-20">🎁</div>
            </div>
            <div
                class="bg-gray-900 p-8 rounded-[3rem] border border-gray-800 shadow-2xl flex justify-between items-center transition-all hover:border-gray-700">
                <div>
                    <p class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] italic leading-none mb-2">
                        Total de Vendas Voucher</p>
                    <h3 class="text-4xl font-black text-white font-mono tracking-tighter italic leading-none">
                        {{ $totalQtd }} <span class="text-sm">Vendas</span></h3>
                </div>
                <div class="text-4xl opacity-20">📊</div>
            </div>
        </div>
        ...
        <div class="bg-gray-900 rounded-[3rem] border border-gray-800 overflow-hidden shadow-2xl">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr
                            class="text-gray-500 text-[10px] font-black uppercase tracking-widest border-b border-gray-800 bg-black/20 italic">
                    ...
                    <tbody class="divide-y divide-gray-800/50 font-sans">
                        @forelse($cortesias as $item)
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="p-6">
                                    <div class="flex flex-col leading-none gap-1">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <span
                                                class="text-[9px] text-orange-500 font-bold uppercase tracking-tighter italic">ID
                                                Mov: #{{ $item->id }}</span>
                                            @if ($item->bar_order_id)
                                                <span
                                                    class="text-[8px] bg-blue-900/30 text-blue-400 px-1.5 rounded uppercase font-black border border-blue-900/50">Mesa
                                                    #{{ $item->bar_order_id }}</span>
                                            @else
                                                <span
                                                    class="text-[8px] bg-emerald-900/30 text-emerald-400 px-1.5 rounded uppercase font-black border border-emerald-900/50">PDV</span>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                <td class="p-6">
                                    <span
                                        class="text-xs text-gray-400 font-bold uppercase italic tracking-tighter">{{ $item->user->name ?? 'Sistema' }}</span>
                                </td>
                                <td class="p-6 text-right">
                                    <span
                                        class="text-orange-600 font-black text-2xl italic font-mono tracking-tighter leading-none">
                                ...
                                <td class="p-6 text-center">
                                    ...
                                    @else
                                        <span class="text-[8px] text-gray-600 italic uppercase font-black">Sem
                                            Itens</span>
                                    @endif
                                </td>
                            </tr>
                ...
                <div class="p-10 relative italic">
                    ...
                    <div x-show="!carregando" class="space-y-6">
                        <div class="bg-black/40 rounded-[2rem] p-6 border border-gray-800/50">
                            <label
                                class="text-[8px] font-black text-gray-600 uppercase tracking-widest block mb-4 italic">Itens
                                Bonificados</label>
                            <div class="space-y-4 max-h-60 overflow-y-auto no-scrollbar">
                                <template x-for="item in venda.itens">
                                    <div
                                        class="flex justify-between items-center border-b border-gray-800 pb-3 last:border-0 leading-none">
                                        <div>
                                            <span class="text-orange-500 font-black text-xs"
                                                x-text="item.qtd + 'x '"></span>
                                            <span class="text-gray-300 text-xs font-bold uppercase italic"
                                                x-text="item.nome"></span>
                                        </div>
                                        <div class="text-right">
                                            <span class="text-white font-mono text-xs font-bold tracking-tighter"
